...
export class ProAdvocate extends BaseAgent {
  async generateArguments(
    framedIssue: FramedIssue, 
    evidence: EvidenceBoard, 
    language: Language,
    focusTopic?: string
  ): Promise<{ pro_speech: ConstructiveSpeech }> {
...
<role>
You are the AFFIRMATIVE CONSTRUCTIVE SPEAKER.
Your goal is to convey logic and evidence as efficiently as possible.
</role>

<style_guidelines>
1. **NO RHETORIC**: ABSOLUTELY FORBIDDEN to use phrases like "Honorable judges," "Ladies and gentlemen," "I firmly believe," or "Allow me to introduce."
2. **CONCISE & DIRECT**: Use a "Gaejo-style" (Outline/Bullet-point) format.
   - Example (Korean): "~함.", "~임.", "~것으로 확인됨." ending style.
   - Example (English): Direct statements. "Inflation risks are high due to X." instead of "We believe that inflation risks are high..."
3. **STRUCTURED OUTPUT**:
   - **Claim**: One clear sentence.
   - **Reasoning**: Logical steps in numbered list or bullet points.
   - **Impact**: The direct consequence.
</style_guidelines>

<structure_requirements>
1. **Introduction:**
   - **Definitions**: Dictionary definition only. No fluff.
   - **Background**: Factual context only.
   - **Value Criterion**: State the standard (e.g., "Standard: Utilitarianism") directly.
   - **Roadmap**: List arguments briefly (e.g., "1. Economy, 2. Ethics").

2. **Body (Contentions):**
   - **Claim**: Direct assertion.
...
`;
         ...
         pro_speech: {
           ...
           properties: {
             introduction: {
               type: Type.OBJECT,
               properties: {
                 hook: { type: Type.STRING, description: "One striking fact or statistic (No greeting)" },
  ...
  async answerUserQuestion(
    ...
    const systemPrompt = `###Instruction###
<role>
You are the AFFIRMATIVE (PRO) SPEAKER.
Your core objective is to defend the Resolution: "${framedIssue.refined_issue}".
</role>

<stance_enforcement>
1. **NEVER CONCEDE**: Even if the user's point is valid or generally true, you MUST pivot back to your stance.
2. **"YES, BUT" LOGIC**: Acknowledge the user's point briefly, then immediately counter it with a "However" or "Nevertheless" argument that supports the PRO side.
   - Example: "It is true that cost is a factor; **however**, the long-term economic benefits outweigh the initial investment because..."
3. **USE EVIDENCE**: Connect your answer to the existing evidence IDs provided.
</stance_enforcement>

<style_guidelines>
1. **NO RHETORIC**: No greetings, no "That is a good question."
2. **FORMAT**: Start directly with the counter-argument/answer. Use bullet points if complex.
3. **TONE**: Unwavering, logical, persuasive.
</style_guidelines>

<task>
1. Answer the user's question.
2. If the user attacks you, defend aggressively.
3. If the user supports you, amplify the impact.
...
`;
    ...
    const schema: Schema = {
      type: Type.OBJECT,
      properties: {
        answer: { type: Type.STRING, description: "Direct answer maintaining PRO stance" }
      },
      required: ["answer"]
    };
    ...
    return this.retryOperation(async () => {
      const response = await this.ai.models.generateContent({
        ...
        contents: `###Context###
Resolution: ${framedIssue.refined_issue}
Pro Position: ${framedIssue.positions.pro}
Evidence: ${JSON.stringify(evidence.evidence_board)}
