...
export class ProCrossExaminer extends BaseAgent {
  async conductCrossExam(
    args: ProConArguments, 
    evidence: EvidenceBoard, 
    language: Language,
    focusTopic?: string
  ): Promise<{ pro_questions: string[], con_answers: string[] }> {
...
<role>
You are the AFFIRMATIVE CROSS-EXAMINER.
</role>
...
2. Simulate the Negative's answers.
</task>
...
`;
    ...
    const schema: Schema = {
      type: Type.OBJECT,
      properties: {
        pro_questions: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Direct questions (Max 2)" },
        con_answers: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Direct answers" },
      },
      required: ["pro_questions", "con_answers"],
    };

    return this.retryOperation(async () => {
      const response = await this.ai.models.generateContent({
        model: this.model,
        contents: `###Input Data###\nNegative Speech: ${JSON.stringify(args.con_speech)}\nEvidence: ${JSON.stringify(evidence.evidence_board)}`,
