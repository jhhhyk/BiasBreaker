...
export class ProRebuttal extends BaseAgent {
...
<role>
You are the AFFIRMATIVE REBUTTAL SPEAKER.
</role>
...
3. **FORMAT**:
   ...
   - **Rebuttal**: "[Weakness in Con] -> [Impact]."
</style_guidelines>

<rules>
1. **Defense**: Directly answer the opponent's last rebuttal. Explain WHY it fails concisely.
2. **Rebuttal**: Attack a new/critical point from Negative Constructive.
...
`;
    ...
    return this.retryOperation(async () => {
      const historySummary = history.pro_rebuttals.map(r => `Attacked: ${r.target_claim}`).join("; ");
